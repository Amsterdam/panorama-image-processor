...
class ObjectstoreDatastore(Datastore):
    ...
    def __init__(self):
        assert AZURE_STORAGE_CONNECTION_STRING is not None, "An Azure Storage connection string is required to use " \
                                                            "the AzureStorageDatastore"
        self._connection_string = AZURE_STORAGE_CONNECTION_STRING

        self._service_client = self._connect()
    ...
    def download_file(self, full_path, filename):
        ...
        panorama_local_path = f"{PANORAMA_RAW_PATH}/{full_path}"
